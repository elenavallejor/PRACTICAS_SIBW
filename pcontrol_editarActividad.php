<?php
   ...
   if (isset($variables['rol'])) { // SI TENEMOS PERMISOS INICIALIZAMOS VARIABLES NECESARIAS
      $idActividad = $_GET['actividad'];
      $variables['actividad'] = getActividad($idActividad);
      $portada = $variables['actividad']['imagen_portada'];
      $galeria = $variables['actividad']['imagenes'];
      $piedeFotoNuevo = $variables['actividad']['piedefoto'];
   
      if ($variables['actividad']['imagenes'] != '' || $variables['actividad']['imagenes'] != NULL) {
   ...
   if ($_SERVER['REQUEST_METHOD'] === 'POST') { 
      ...
      $idEventoAEditar = $_POST['idActividadEditar'];
      ...
      modifyActividad($idEventoAEditar, $tituloNuevo, $precioNuevo, $fechaNueva, $descripcionNueva, $galeria, $piedeFotoNuevo, $portada, $hashtagsNuevo);
   ...
   if (isset($variables['rol']) && $variables['rol'] >= 3) { // Si el usuario tiene permisos
      echo $twig->render('pcontrol_editarActividad.html', $variables);
   } else {
